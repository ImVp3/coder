...
def build_supervisor_agent(supervisor_model: str|BaseChatModel, worker_model: str|BaseChatModel, retriever):
        ...
        return Command(
            ...
            update={
                "messages": state["messages"] + [
                    ...
                    final_answer  # thường là AIMessage
                ]
            }
        )
    # Define workers
    ...
    supervisor_agent = create_react_agent(
        ...
        tools=[assign_to_research_agent, assign_to_math_agent, synthesis_tool],
        prompt=(
        ...
        "When you have collected all relevant outputs, call the `synthesis tool` to synthesize the final answer."
        ),
        name="supervisor",
    )
    ...
    supervisor_graph.add_node(supervisor_agent, destinations=("codegen_agent", "testgen_agent", END))
